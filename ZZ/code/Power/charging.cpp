...
void polnjenje(void *paramOdTaska)
{
	while (true)
	{
		if (Hardware.battery_voltage >= 4150 && Hardware.status_reg.charging_finished == 0)
		{
			...
			EEPROM.pisi(Hardware.status_reg.charging_finished, battery_stat_addr); //Posodobitev EEPROM-a na bajtu 1 z spremenljivko readBIT(Hardware.status_reg, HARDWARE_STATUS_REG_CHARGING_EN); Na vsake 5000 pisanj zamenja bajt na katerega piše
		}
		...
		{ //Če se dokonca napolne, se bo polnjenje lahko nadaljevalo šele, ko se baterija izprazne za približno 10% (3V = 0%, 4.2V = 100%, 4.1V = 90% . 3.95V = 80% oz. 10% manj ;  napetost = 0.012 * procent + 3);
			...
			EEPROM.pisi(Hardware.status_reg.charging_finished, battery_stat_addr);
		}
